...
then
    if [ -z $SRA_ARRAY ]
    then 
        echo "$PROJECT_DIR/$SAMPLE_NAME/data is empty or does not exist and --sra-array= was not used to specify SRA file(s) or download. data/ directory must contain SRA or FASTQ files to run Majel.py"
    else
        mkdir -p $DATA_DIR
        SRA_ARRAY=($SRA_ARRAY)
fi
