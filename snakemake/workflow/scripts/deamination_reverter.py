...
def process_bam_segment(args):
    (bam_path, bed_file_path, revert_path, revert_to, check_base, 
    ...
    with pysam.AlignmentFile(bam_path, "rb") as bam:
        ...
        for read in bam.fetch(contig, start, end):
            ...
            revert_qual = list(read.query_qualities)
            ...
            for idx, pos in enumerate(read.get_reference_positions(full_length=True)):
                ...
                if overlapping_segments:                  
                    ...
                    CpG_end = CpG_coords.end - 1
                    logging.debug(f"CpG overlap: {pos}, CpG_start: {CpG_start}, CpG_end: {CpG_end}, pos: {pos}")

                    if pos == CpG_end and revert_to == 'C' and revert_seq[idx] == 'G':
                        revert_seq[idx] = 'N'
                        revert_qual[idx] = 0
                    elif pos == CpG_start and revert_to == 'G' and revert_seq[idx] == 'C':
                        revert_seq[idx] = 'N'
                        revert_qual[idx] = 0

                # Otherwise if a non-CpG site matches expected conversion by deamination,
                # revert this base to the reference
                elif ref_base == revert_to and revert_seq[idx].upper() == check_base:
                    logging.debug(f"revert_to: {revert_to}")
                    revert_seq[idx] = revert_to
                    reverted_count += 1

            read.query_sequence = ''.join(revert_seq)
            read.query_qualities = pysam.array.array('B', revert_qual)
...
def split_bam_processing(bam_path, ref_genome_path, num_threads, revert_to, check_base, bed_file_paths,
                         target_reads_per_segment, chromosome_group, merged_reverted_path):
    ...
    with pysam.AlignmentFile(bam_path, "rb") as bam:
        ...
        for segment in segments:
            ...
            if contig in bed_file_paths:
                ...
                job = (bam_path, bed_file_paths[contig], revert_path, revert_to, check_base,
