...
option_list = list(
  make_option(c("-o", "--output_dir"), type="character", default=".",
              help="Path to file output directory", metavar="character"),
...
if(opt$genome == "hg38" | opt$genome == "hg38_old"){
  opt$genome = "hg38"
...
out_prefix = paste(opt$output_dir, gsub("_sd_CpG.bedGraph", "", bGraph_name), sep = "/")
#1st check the coverage, minimum needed is 10x
covFile = gsub("_CpG.bedGraph", "_conversion_and_coverage.txt", bGraph)
covFile = gsub("05_call_methylation", "07_calculate_statistics", covFile)
...
#now to do the tdf
tdf_file = paste(opt$output_dir, gsub("bedGraph$", "tdf", bGraph_name), sep = "/")
