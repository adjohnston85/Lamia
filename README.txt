...
or when using SRA as input. Due 
to the way ruffus works and the occasional need for error recovery, most intermediate files cannot be deleted on the fly. 
Instead, these files can be removed and the final files put into 
a tidy directory structure using the cleanup script. When using SRA as input, the user is 
